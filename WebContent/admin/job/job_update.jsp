		...
		<div class="page-content">
			<div class="panel panel-default">
				...
				<div class="panel-body">
					<form action="<%=path%>/job_update.action" method="post"
						id="addForm">
						<input name="id" type="hidden" value="${job.id}" />
						<input name="bid" type="hidden" value="${job.bid}" />
						<input name="zt" type="hidden" value="${job.zt}" />
						<input name="cdate" type="hidden" value="${job.cdate}" />
									...
									<div class="form-group">
										...
										<div class="col-sm-9 form-inline">
											<input id="name" name="name" size="35" class="form-control"
												type="text" value="${job.name}" tip="请输入标题" />
										</div>
									</div>
								</div>
							</div>
										...
										<div class="col-sm-9 form-inline">
											<select id="catelogid" name="catelogid" class="form-control">
												<c:forEach items="${catelogList}" var="info">
													<option
														<c:if test="${job.catelogid==info.id}">selected</c:if>
														value="${info.id}">
														${info.name}
													</option>
												</c:forEach>
									...
									<div class="form-group">
										...
										<div class="col-sm-9 form-inline">
											<input id="minsalary" name="minsalary" size="35" style="width:80px"
												class="form-control" type="text" value="${job.minsalary}"
												tip="请输入最小薪资" />
											-	
											<input id="maxsalary" name="maxsalary" size="35" style="width:80px"
												class="form-control" type="text" value="${job.maxsalary}"
												tip="请输入最大薪资" />	
										</div>
									</div>
								</div>
									...
									<div class="form-group">
										...
										<div class="col-sm-9 form-inline">
											<input id="address" name="address" size="35"
												class="form-control" type="text" value="${job.address}"
												tip="请输入工作地址" />
										</div>
									</div>
								</div>
									...
									<div class="form-group">
										...
										<div class="col-sm-9 form-inline">
											<input id="quyu" name="quyu" size="35" class="form-control"
												type="text" value="${job.quyu}" tip="请输入工作区域" />
										</div>
									</div>
								</div>
							</div>
									...
									<div class="form-group">
										...
										<div class="col-sm-9 form-inline">
											<select id="xueli" name="xueli" class="form-control">
												<option <c:if test="${job.xueli=='初中及以下'}">selected</c:if> value="初中及以下">
													初中及以下
												</option>
												<option <c:if test="${job.xueli=='高中'}">selected</c:if> value="高中">
													高中
												</option>
												<option <c:if test="${job.xueli=='大专'}">selected</c:if> value="大专">
													大专
												</option>
												<option <c:if test="${job.xueli=='本科'}">selected</c:if> value="本科">
													本科
												</option>
												<option <c:if test="${job.xueli=='硕士'}">selected</c:if> value="硕士">
													硕士
												</option>
												<option <c:if test="${job.xueli=='博士'}">selected</c:if> value="博士">
													博士
												</option>
											</select>
									...
									<div class="form-group">
										...
										<div class="col-sm-9 form-inline">
											<input id="jingyan" name="jingyan" size="35"
												class="form-control" type="text" value="${job.jingyan}"
												tip="请输入工作经验" />
										</div>
									</div>
								</div>
