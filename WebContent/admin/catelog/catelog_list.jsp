...
<html>
	...
	<body>
		<div class="page-content">
			<form action="<%=path%>/catelog_list.action" class="form-inline"
				method="post">
				<div class="panel panel-default">
					...
					<pg:pager url="catelog_list.action" items="${itemSize}"
										...
										<tr>
											<td>
												<a style="color: #000000"
													href="<%=path%>/catelog_toView.action?id=${info.id}">${info.name}</a>
											</td>
											<td>
												${info.miaoshu}
											</td>
											<td>
												<a href="<%=path%>/catelog_toUpdate.action?id=${info.id}"
...
$(document).ready(function(){
   $("#addBtn").click(function(){
        window.location.href = '<%=path%>/catelog_toAdd.action';
   });
});
function delInfo(id){
	 if(confirm("是否确认删除？")){
	 window.location.href = '<%=path%>/catelog_delete.action?id=' + id;
    }
}
</script>